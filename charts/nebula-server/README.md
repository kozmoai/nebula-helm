...
Nebula server application bundle

**Homepage:** <https://github.com/kozmoai>

## Installing the Chart

To install the chart with the release name `nebula-server`:
...
Without making any modifications to the `values.yaml` file, you can access the Nebula UI by port forwarding either the Server `pod` or `service` with the following command and visiting [http:localhost:4200](http:localhost:4200):
```console
kubectl port-forward svc/nebula-server 4200:4200
```

Note: If you choose to make modifications to either the `server.nebulaApiUrl` or `service.port`, make sure to update the other value with the updated port!
...
1. `postgresql.auth.password`: a password you want to set for the nebula user
...
1. `nebula-server-postgresql-connection`: used by the nebula-server deployment to connect to the postgresql database.

2. `<release-name>-postgresql-0`: defines the `postgresql.auth.username`'s password on the postgresql server to allow successful authentication from the nebula server.

No secrets are created when providing an existing secret.

### Connecting with SSL configured

1. Mount the relevant certificate to `/home/nebula/.postgresql` so that it can be found by `asyncpg`. This is the default location postgresql expects per their [documentation](https://www.postgresql.org/docs/current/libpq-ssl.html).
    ```yaml
    nebula-server:
      server:
        extraVolumes:
        ...
        extraVolumeMounts:
          - name: db-ssl-secret
            mountPath: "/home/nebula/.postgresql"
            readOnly: true
      postgresql:
        useSubChart: false
        auth:
...
| fullnameOverride | string | `"nebula-server"` | fully override common.names.fullname |
...
| ingress.host.hostname | string | `"nebula.local"` | default host for the ingress record |
...
| postgresql.auth.password | string | `"nebula-rocks"` | password for the custom user. Ignored if `auth.existingSecret` with key `password` is provided |
| postgresql.auth.username | string | `"nebula"` | name for a custom user |
...
| server.image.nebulaTag | string | `"2-latest"` | nebula image tag (immutable tags are recommended) |
...
| server.image.repository | string | `"kozmoai/nebula"` | server image repository |
...
| server.nebulaApiHost | string | `"0.0.0.0"` | sets NEBULA_SERVER_API_HOST |
| server.nebulaApiUrl | string | `"http://localhost:4200/api"` | sets NEBULA_API_URL |
...
| server.uiConfig.enabled | bool | `true` | set NEBULA_UI_ENABLED; enable the UI on the server |
| server.uiConfig.nebulaUiApiUrl | string | `""` | sets NEBULA_UI_API_URL |
| server.uiConfig.nebulaUiStaticDirectory | string | `"/ui_build"` | sets NEBULA_UI_STATIC_DIRECTORY |
| server.uiConfig.nebulaUiUrl | string | `""` | sets NEBULA_UI_URL |
...
| service.targetPort | int | `4200` | target port of the server pod; also sets NEBULA_SERVER_API_PORT |
